...
class SaveModeAwarenessDetectorTest : LintDetectorTest() {
    ...
    fun testBatteryChangedInConstructor() {
        lint().ecolint().files(
            ...
            src/com/example/Example.kt:6: Warning: Taking into account when the device is entering or exiting the power save mode is a good practice. [SaveModeAwareness]
                                    val filter = IntentFilter("android.intent.action.BATTERY_CHANGED")
                                                              ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            0 errors, 1 warnings
        """.trimIndent())
    ...
    fun testBatteryChangedViaAddAction() {
        lint().ecolint().files(
            ...
            src/com/example/Example.kt:7: Warning: Taking into account when the device is entering or exiting the power save mode is a good practice. [SaveModeAwareness]
                                    filter.addAction("android.intent.action.BATTERY_CHANGED")
                                                     ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            0 errors, 1 warnings
        """.trimIndent())
    ...
    fun testIsPowerSaveMode() {
        lint().ecolint().files(
            ...
            src/com/example/PowerCheck.kt:6: Warning: Taking into account when the device is entering or exiting the power save mode is a good practice. [SaveModeAwareness]
                                    val isSaving = pm.isPowerSaveMode
                                                   ~~~~~~~~~~~~~~~~~~
            0 errors, 1 warnings
        """.trimIndent())
