...
class KeepScreenOnFlagsDetectorTest : LintDetectorTest() {
    ...
    fun testSetFlagKeepScreenOn() {
        lint().ecolint().files(
            ...
            src/test.kt:4: Warning: Setting FLAG_KEEP_SCREEN_ON may unnecessarily keep screen and CPU active. Using FLAG_KEEP_SCREEN_ON can prevent the device from sleeping, which drains battery. [KeepScreenOnSetFlag]
                            fun test(window: Window) {
                            ^
            0 errors, 1 warnings
        """.trimIndent())
    ...
    fun testAddFlagKeepScreenOn() {
        lint().ecolint().files(
            ...
            src/test.kt:3: Warning: Setting FLAG_KEEP_SCREEN_ON may unnecessarily keep screen and CPU active. Using FLAG_KEEP_SCREEN_ON can prevent the device from sleeping, which drains battery. [KeepScreenOnSetFlag]
                            fun test(flags: Int, params: WindowManager.LayoutParams) {
                            ^
            0 errors, 1 warnings
        """.trimIndent())
