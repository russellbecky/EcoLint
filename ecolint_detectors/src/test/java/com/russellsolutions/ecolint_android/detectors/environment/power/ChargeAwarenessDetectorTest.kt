...
class ChargeAwarenessDetectorTest : LintDetectorTest() {
    ...
    fun testPowerBroadcastActions() {
        ...
        ).skipTestModes(TestMode.SUPPRESSIBLE).run().expectWarningCount(2).expect("""
            src/com/example/PowerListener.kt:7: Warning: Monitoring power changes and customizing behavior depending on battery level is a good practice. [🌱 EcoLint: ChargeAwarenessIntentUsage]
                                    filter.addAction("android.intent.action.ACTION_POWER_CONNECTED")
                                                     ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            src/com/example/PowerListener.kt:8: Warning: Monitoring power changes and customizing behavior depending on battery level is a good practice. [🌱 EcoLint: ChargeAwarenessIntentUsage]
                                    filter.addAction("android.intent.action.BATTERY_LOW")
                                                     ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            0 errors, 2 warnings
        """.trimIndent())
    ...
    fun testConstructorUsage() {
        lint().ecolint().files(
            ...
            src/com/example/BatteryReceiver.kt:6: Warning: Monitoring power changes and customizing behavior depending on battery level is a good practice. [🌱 EcoLint: ChargeAwarenessIntentUsage]
                                    val filter = IntentFilter("android.intent.action.BATTERY_OKAY")
                                                              ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            0 errors, 1 warnings
        """.trimIndent())
